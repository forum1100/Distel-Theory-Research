...
# Optimized merged DT simulation:
# bounded persistence + flourishing/resilience + controls + ablations + shocks

N = 80
T_STEPS = 900
SEEDS = range(300, 320)  # 20 seeds to complete within runtime
modes = [
...
def run_merged(mode, seed):
    ...
    # save every 5 cycles for performance
    history = []
    ...
    for t in range(T_STEPS):
        ...
        # bounded persistence substrate, applied to every branch
        ...
        P = np.clip(P + base_p + memory_pull + variance_damping + lockin_penalty, 0, 100)
        R = 0.96*R + 0.04*P
        ...
        if mode == "centralized_optimization":
            ...
            corr = 0.030 * drivers[:,4] * drivers[:,1] * (1 + P/(1+P))

            # vectorized dominant destabilizer correction
            rows = np.arange(N)
...
for mode in modes:
    ...
    for seed in SEEDS:
        ...
        final = s.iloc[-1]
        # recovery windows approximate using sampled cycles
        recoveries = []
        for start, end in [(290,390),(510,610),(710,810),(840,899)]:
            w = s[(s["cycle"]>=start)&(s["cycle"]<end)]
            recoveries.append(w["system_F"].mean())
        rows.append({
            "mode": mode,
            "seed": seed,
            ...
            "avg_recovery_F": float(np.mean(recoveries)),
            "min_recovery_F": float(np.nanmin(recoveries)),
...
print(summary[["mode","final_F_mean","low_tail_F_mean","avg_recovery_F","final_mean_P","final_max_P","destab_mean","final_lockin","final_alienation"]].to_string(index=False))
